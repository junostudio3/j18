...
class jConfigParser:
    ...
    def ReadText(self, category:str, key:str, default_value:str):
        if not category in self.file: return default_value
        if not key in self.file[category]: return default_value

        return self.file[category][key]

    def WriteBoolean(self, category:str, key:str, value:bool, default_value:bool):
        self.WriteInt(category, key, int(value), int(default_value))

    def WriteInt(self, category:str, key:str, value:int, default_value:int):
        self.WriteText(category, key, str(value), str(default_value))

    def WriteText(self, category:str, key:str, value:str, default_value:str):
        if value== default_value: return    # Default Value는 저장하지 않는다
        if not category in self.file: self.file[category] = {}
